...
import { EXPENSE_RECURRENCES } from "@/services/expenses";
import { PO_EXPENSABLE_STATUSES } from "../../purchase-orders/constants";
...
export default async function NewExpensePage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const [{ buildings, units }, vendors, workOrders, purchaseOrders, settings] = await Promise.all([
    locationOptions(ctx),
    ...
    db.purchaseOrder.findMany({
      where: { AND: [byPropertyWhere(ctx), { OR: [{ status: { in: [...PO_EXPENSABLE_STATUSES] } }, ...(str(sp.purchaseOrderId) ? [{ id: str(sp.purchaseOrderId) }] : [])] }] },
      orderBy: { createdAt: "desc" },
      take: 300,
      select: { id: true, number: true, description: true, amount: true, propertyId: true, vendorId: true, status: true },
    }),
    getOrgSettings(ctx.organizationId),
  ]);
  const presetWo = workOrders.find((w) => w.id === str(sp.workOrderId));
  const presetPo = purchaseOrders.find((p) => p.id === str(sp.purchaseOrderId));
        ...
        <ActionForm action={createExpenseAction}>
          <div className="grid gap-4 sm:grid-cols-2">
            <LocationPicker
              ...
              defaultPropertyId={presetWo?.propertyId ?? presetPo?.propertyId ?? undefined}
            ...
            <Select
              label={t("exp.purchaseOrder")}
              name="purchaseOrderId"
              defaultValue={presetPo?.id ?? ""}
              placeholder={t("common.none")}
              hint={t("exp.poHint")}
              options={purchaseOrders.map((p) => ({ value: p.id, label: `${p.number} — ${p.description} (${money(p.amount).toFixed(0)})` }))}
            />
            <Select label={t("exp.vendor")} name="vendorId" defaultValue={presetWo?.vendorId ?? presetPo?.vendorId ?? ""} placeholder={t("common.none")} options={vendors.map((v) => ({ value: v.id, label: v.name }))} />
            ...
            <Input label={t("common.amount")} name="amount" type="number" min="0.01" step="0.01" inputMode="decimal" required defaultValue={presetPo ? money(presetPo.amount).toFixed(2) : undefined} />
            ...
            <Select label={t("exp.recurrence")} name="recurrence" defaultValue="NONE" hint={t("exp.recurrenceHint")} options={EXPENSE_RECURRENCES.map((v) => ({ value: v, label: t(`exp.rec.${v}`) }))} />
          </div>
          <Input label={t("exp.description")} name="description" required minLength={2} maxLength={500} defaultValue={presetWo ? `${presetWo.number} — ${presetWo.title}` : presetPo ? `${presetPo.number} — ${presetPo.description}` : ""} />
