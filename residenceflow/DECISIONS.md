...
| D-15 | Tenants are visible to building-scoped staff through their **leases**, plus tenants that have no lease yet (so a manager can create a tenant and then a lease). | Once a tenant has a lease, only staff of that building see them. A `homePropertyId` column would tighten this further. |
